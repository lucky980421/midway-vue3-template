...
export async function request(axiosConfig: Config) {
  // 自定义配置
  const options = Object.assign(
    {
      debounceRequest: false, // 是否开启取消重复请求, 默认为 true
    },
    axiosConfig
  );
  const result: any = await service(options);
  if (result.ret !== 0) {
    throw result?.msg;
  }
  return result.data;
